...
@app.route("/display_wordcloud/<string:tweet_id>")
def view_wordcloud(tweet_id):
    image_path = url_for("static", filename=f"{tweet_id}.jpg")
    return render_template(
        "display_image.html", image_path=image_path, tweet_id=tweet_id
    )
...
scheduler.add_job(func=reply_tweets, trigger="interval", seconds=60)
scheduler.start()
